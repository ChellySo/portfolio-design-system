...
const Card: React.FC<CardProps> = ({ className, title, href, children, date, ...rest }: CardProps) => {
    return (
        <div className={classNames(!!date ? 'rounded-8 xs:grid xs:grid-flow-col xs:auto-cols-auto xs:gap-4' : 'rounded-8', className)} {...rest}>
            {date &&
            ...
            <div>
                <div className="flex text-purple-200">
                    <Heading variant="h4" className="leading-6 xs:text-heading-3">
                        {title}
                    </Heading>
